...
export function NewReleasePage() {
  ...
  return (
    <>
      <PageHeader title="새 릴리즈 등록" description="표준 패키지를 등록하면 SHA256 검증과 안전한 압축 해제를 거쳐 배포 파이프라인이 준비됩니다." crumbs={[{ label: '릴리즈', to: '/releases' }, { label: '새 릴리즈' }]} />
              ...
              <Stack spacing={3}>
                ...
                <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
                  <FormControl fullWidth required>
                    <InputLabel id="application-label">애플리케이션</InputLabel>
                    <Select labelId="application-label" label="애플리케이션" value={applicationId} onChange={(event) => { setApplicationId(event.target.value); setEnvironmentId(''); setProfileId(''); }}>
                  ...
                  <FormControl fullWidth required>
                    <InputLabel id="environment-label">대상 환경</InputLabel>
                    <Select labelId="environment-label" label="대상 환경" value={environmentId} onChange={(event) => { setEnvironmentId(event.target.value); setProfileId(''); }}>
                ...
                <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
                  ...
                  <FormControl fullWidth>
                    <InputLabel id="profile-label">배포 프로필</InputLabel>
                    <Select labelId="profile-label" label="배포 프로필" value={profileId} onChange={(event) => setProfileId(event.target.value)} disabled={!applicationId || !environmentId}>
                ...
                <Box sx={{ pt: 1 }}>
                  ...
                  <Box
                    ...
                    sx={{
                      ...
                      bgcolor: file ? alpha('#68a9ff', 0.08) : alpha('#07101f', 0.35),
                      borderRadius: 2.5,
                  ...
                  >
                    ...
                    <Box>
                      ...
                      <Typography color="text.secondary" variant="body2" sx={{ mt: 0.5 }}>
                        {file ? formatBytes(file.size) : '.tar, .tar.gz 파일 지원'}
                      </Typography>
                    </Box>
                  </Box>
